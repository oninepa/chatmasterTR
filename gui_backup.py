...
class ChatManagerGUI:
    ...
    def create_right_panel(self):
        # 새 대화 업로드 준비
        ttk.Label(self.right_top, text="새 대화 업로드 준비", font=('맑은 고딕', 12, 'bold')).pack(anchor="w")
        
        ttk.Label(self.right_top, text="주제:").pack(anchor="w", pady=(10, 5))
        self.topic_entry = ttk.Entry(self.right_top)
        self.topic_entry.pack(fill="x")

        self.save_prep_text = tk.Text(self.right_top, wrap="word", height=10)
        self.save_prep_text.pack(fill="both", expand=True, pady=10)

        ttk.Button(self.right_top, text="업로드 (저장소로)").pack(anchor="e")

        # 저장된 대화 검색 및 관리
        ttk.Label(self.right_bottom, text="저장된 대화 검색 및 관리", font=('맑은 고딕', 12, 'bold')).pack(anchor="w", pady=(20, 10))

        search_frame = ttk.Frame(self.right_bottom)
        search_frame.pack(fill="x", pady=(0, 10))

        ttk.Label(search_frame, text="저장소 선택:").pack(side="left")
        self.storage_select = ttk.Combobox(search_frame, values=["Firebase"], width=15)
        self.storage_select.pack(side="left", padx=5)
        self.storage_select.set("Firebase")

        ttk.Label(search_frame, text="키워드 검색:").pack(side="left", padx=(10, 0))
        self.keyword_entry = ttk.Entry(search_frame, width=20)
        self.keyword_entry.pack(side="left", padx=5)

        # 대화 목록 Treeview
        self.conversation_tree = ttk.Treeview(self.right_bottom, columns=("date", "model", "topic"), show="headings")
        ...
        # 미리보기/편집 영역
        self.preview_text = tk.Text(self.right_bottom, wrap="word", height=10)
        self.preview_text.pack(fill="both", expand=True, pady=10)

        # 하단 버튼들
        button_frame = ttk.Frame(self.right_bottom)
